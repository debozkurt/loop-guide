...
This is the supervisor/worker topology from Ch 10 made runnable:
  ...
  - a fleet-level budget caps TOTAL spend, because per-worker caps hide a runaway
    fleet (Ch 13, Ch 14)
...
class FleetConfig:
    ...
    fleet_budget_usd: float = 50.0  # TOTAL across all workers (Ch 13, 14)
...
def run_worker(task_id: int, task: str, fc: FleetConfig) -> tuple[int, str, loop.StopReason]:
    ...
    try:
        cfg = dataclasses.replace(fc.per_worker, repo_dir=wt, dry_run=fc.dry_run)
